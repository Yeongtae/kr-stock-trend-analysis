...
- 당일 수급: 다음 금융 `TODAY`에 요청 종료일 데이터가 있으면 사용, 없거나 날짜가 다르면 KRX
- 최근 5거래일 수급: 다음 금융 `DAYS_5`에 요청 종료일 데이터가 있으면 사용, 없거나 날짜가 다르면 KRX
- 최근 3거래일 수급: KRX의 정확한 거래일 구간 조회
- 다음 금융 응답의 `toDate`(당일은 `fromDate`도 함께)를 요청 종료일과 검증한 뒤 fallback 여부를 결정
...
- `raw/YYYY-MM-DD/`: 구성종목·가격·다음 금융 성공 응답·KRX fallback JSON
